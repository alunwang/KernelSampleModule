...
#define SAMPLE_VENDOR_ID 0x666
#define SAMPLE_DEVICE_ID 0x999
static struct pci_device_id sample_ids[] = {
    {SAMPLE_VENDOR_ID, SAMPLE_DEVICE_ID, PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0},
    {0,}
};
MODULE_DEVICE_TABLE(pci, sample_ids);
...
static int __init sample_drv_init(void)
{
	int ret;
	...
	/* 申请动态设备号 */
	ret = alloc_chrdev_region(&my_device.devno, 0, DEVICE_NUMBER, DEV_NAME);
	if (ret < 0) {
...
static void __exit sample_drv_exit(void)
{
